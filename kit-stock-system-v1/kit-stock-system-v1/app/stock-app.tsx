...
const navItems: Array<{ id: Tab; label: string; icon: string }> = [
  ...
  { id: "scan", label: "สแกน Tag", icon: "⌁" },
  { id: "receive", label: "รับงาน", icon: "✓" },
    ...
    <div className="app-shell">
      <aside className="sidebar">
        <div className="brand-block">
          <div className="brand-mark">KIT</div>
          <div>
            <strong>Stock Verify</strong>
      ...
      <main className="main-area">
        <header className="topbar">
          <div>
            <p className="eyebrow">KIT • KISHIMOTO INDUSTRY (THAILAND)</p>
            <h1>{tabLabels[tab]}</h1>
          </div>
          <div className="topbar-actions">
            <span className="online-pill"><i /> ระบบพร้อมใช้งาน</span>
            <span className="signed-user"><b>{user.displayName.slice(0, 1)}</b><span>เข้าสู่ระบบแล้ว<strong>{user.displayName}</strong></span></span>
            <button className="notification-button" aria-label="การแจ้งเตือน">●</button>
          </div>
        </header>
          ...
          <section className="page-stack">
            <div className="scan-hero">
              <div className="scan-hero-copy">
                <span className="hero-kicker">QUICK SCAN</span>
                <h2>เริ่มตรวจสอบบ๊อคงาน</h2>
                <p>ยิงบาร์โค้ดใบงานเพื่อเปิด Part, Lot และจำนวนที่ต้องตรวจ</p>
                <form className="order-scan" onSubmit={openOrder}>
                  <span className="barcode-glyph">▥</span>
                  <input value={orderQuery} onChange={(event) => setOrderQuery(event.target.value)} aria-label="Work Order" />
                  <button>เปิดใบงาน</button>
                </form>
              </div>
              <div className="hero-visual" aria-hidden="true">
                <span className="scan-line" />
                <span className="barcode-large">▥ ▥ ▥</span>
            ...
            <div className="metric-grid">
              <article className="metric-card blue">
                <span>ใบงานกำลังตรวจ</span><strong>{data.orders.filter((order) => order.status === "in_progress").length}</strong><small>รายการวันนี้</small>
              </article>
              <article className="metric-card green">
                <span>Tag ที่ตรวจแล้ว</span><strong>{data.scans.length}</strong><small>มีรูปยืนยันครบ</small>
              </article>
              <article className="metric-card amber">
                <span>จำนวนรวมที่ตรวจ</span><strong>{formatNumber(data.scans.reduce((sum, scan) => sum + scan.actualQty, 0))}</strong><small>ชิ้น</small>
              </article>
              <article className="metric-card violet">
                <span>รายการผิดปกติ</span><strong>{data.orders.filter((order) => order.status === "over").length}</strong><small>ต้องตรวจสอบ</small>
              </article>
            </div>
            ...
            <div className="content-grid dashboard-grid">
              <article className="panel">
                <div className="panel-heading">
                  <div><span className="section-kicker">ACTIVE WORK ORDERS</span><h3>ใบงานล่าสุด</h3></div>
                  <div className="heading-actions"><button className="primary-small" onClick={openNewOrder}>+ สร้างใบส่งงาน</button><button className="text-button" onClick={() => setTab("history")}>ดูประวัติทั้งหมด →</button></div>
                </div>
                <div className="order-list">
                  {data.orders.map((order) => {
                    const scans = data.scans.filter((scan) => scan.workOrderId === order.id);
                    const total = scans.reduce((sum, scan) => sum + scan.actualQty, 0);
                    const percent = Math.min((total / order.targetQty) * 100, 100);
                    return (
                      <button className="order-row" key={order.id} onClick={() => chooseOrder(order)}>
                        <ProductArtwork small imageUrl={order.imageUrl} />
                        <div className="order-main"><strong>{order.partNo}</strong><span>{order.partName}</span><small>{order.orderNo} • {order.lotNo}</small></div>
                        <div className="order-progress"><div><span>ตรวจแล้ว</span><strong>{formatNumber(total)}/{formatNumber(order.targetQty)}</strong></div><div className="mini-progress"><i style={{ width: `${percent}%` }} /></div></div>
              ...
              <aside className="panel activity-panel">
                <div className="panel-heading"><div><span className="section-kicker">RECENT ACTIVITY</span><h3>ตรวจล่าสุด</h3></div></div>
                {data.scans.length ? data.scans.slice(0, 4).map((scan) => (
                  <div className="activity-item" key={scan.id}>
                    <img src={scan.photoUrl} alt={`รูปยืนยัน ${scan.tagId}`} />
                    <div><strong>{scan.partNo}</strong><span>{scan.boxType === "full" ? "บ๊อคเต็ม" : "บ๊อคเศษ"} • {scan.actualQty} ชิ้น</span><small>{formatDate(scan.createdAt)}</small></div>
                    <i>✓</i>
                  </div>
                )) : <div className="empty-mini">ยังไม่มีรายการตรวจ<br />เริ่มจากยิง Tag ใบงานด้านบน</div>}
              </aside>
            </div>
          </section>
        )}
