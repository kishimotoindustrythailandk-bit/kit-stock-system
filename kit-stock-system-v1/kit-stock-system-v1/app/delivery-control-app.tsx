...
function PartImagePair({ materialCode, masterVersion, actualVersion, masterAvailable = true, actualAvailable = true, projectionVersion = EFFECTIVE_IMAGE_PROJECTION_VERSION, strictSlots = false, swapSources = false }: { materialCode: string; masterVersion?: string; actualVersion?: string; masterAvailable?: boolean; actualAvailable?: boolean; projectionVersion?: string; strictSlots?: boolean; swapSources?: boolean }) {
  ...
  // Existing production images were saved under the opposite internal slots.
  // Scan popups can swap only their sources while keeping the user-facing labels and order correct.
  const actualSlot = swapSources ? "master" : "actual";
  const masterSlot = swapSources ? "actual" : "master";
  const actualSourceVersion = swapSources ? masterVersion : actualVersion;
  const masterSourceVersion = swapSources ? actualVersion : masterVersion;
  return <div className="part-image-pair" style={{ display: "flex", gap: 12, flexWrap: "wrap", width: "100%" }}>
    <figure style={figStyle}><figcaption style={capStyle}>รูปชิ้นงานในกล่อง</figcaption>{actualAvailable ? <PartImage materialCode={materialCode} slot={actualSlot} version={actualSourceVersion || projectionVersion} strict={strictSlots} /> : fallback}</figure>
    <figure style={figStyle}><figcaption style={capStyle}>รูปตัวอย่าง (Master)</figcaption>{masterAvailable ? <PartImage materialCode={materialCode} slot={masterSlot} version={masterSourceVersion || projectionVersion} strict={strictSlots} /> : fallback}</figure>
  </div>;
}
  ...
  return <div className="control-shell">
    ...
    {replacementPreview && <div className="modal-backdrop replacement-confirm-backdrop" role="dialog" aria-modal="true" aria-labelledby="replacement-confirm-title"><div className="replacement-confirm-modal">
      ...
      <div className="replacement-confirm-content">
        <section className="replacement-confirm-photo dispatch-confirm-images"><PartImagePair materialCode={replacementPreview.request.materialCode} masterVersion={partImages.find((item) => item.materialCode === replacementPreview.request.materialCode)?.updatedAt} actualVersion={partActualImages.find((item) => item.materialCode === replacementPreview.request.materialCode)?.updatedAt} strictSlots swapSources /></section>
