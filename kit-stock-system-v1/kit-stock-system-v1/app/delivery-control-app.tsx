...
export default function DeliveryControlApp({ user, signOutPath }: { user: { id: number; employeeCode: string; displayName: string; email: string; role: string; permissions: PageKey[] }; signOutPath: string }) {
  ...
  async function printStockTags(input: StockTag | StockTag[]) {
    ...
    popup.document.write(`<!doctype html><html lang="th"><head><meta charset="utf-8"><title>${html(tags[0].jobNo)} · ${fmt(tags.length)} Tag</title><style>
      @page{size:A4 portrait;margin:4mm}*{box-sizing:border-box}html,body{margin:0;width:100%;height:100%;font-family:Arial,"Noto Sans Thai",sans-serif;color:#10264c}
      .sheet{height:289mm;display:grid;grid-template-columns:repeat(2,1fr);grid-template-rows:repeat(4,1fr);gap:2mm;break-after:page}.sheet:last-child{break-after:auto}.tag{min-width:0;min-height:0;border:1.1px solid #0a61d8;border-radius:2mm;overflow:hidden;display:grid;grid-template-rows:auto auto 1fr auto;break-inside:avoid;background:#fff}
      header{display:flex;justify-content:space-between;align-items:center;padding:1mm 1.6mm;color:#fff;background:#075fd7}.brand{font-size:14px;font-weight:900;line-height:.8}.brand small{display:block;margin-top:.7mm;font-size:3.5px;letter-spacing:.6px;color:#dbeaff}.tag-title{text-align:right}.tag-title b{display:block;font-size:6px;letter-spacing:.25px}.tag-title small{font-size:4.4px;color:#dbeaff}
      .product{display:grid;grid-template-columns:19mm 17mm 1fr;gap:1.2mm;padding:1mm 1.6mm;border-bottom:1px solid #cbd9eb;background:#f5f9ff}.photo-wrap{height:18mm;display:grid;place-items:center;border:1px solid #b9cce5;border-radius:1.3mm;background:#fff;overflow:hidden}.photo{width:100%;height:100%;object-fit:contain}.photo-fallback{color:#8493aa;text-align:center;font-size:4.4px}.photo-fallback strong{display:block;font-size:12px;color:#b7c5d8}.qr-wrap{height:18mm;display:grid;place-items:center;align-content:center;border:1px solid #b9cce5;border-radius:1.3mm;background:#fff}.qr{width:14mm;height:14mm}.qr-wrap small{font-size:3.2px;color:#526783}
      .main{align-self:center;min-width:0}.main small{font-size:4px;letter-spacing:.2px;color:#6d7e98}.main b{display:block;margin:.25mm 0;font-size:8px;overflow-wrap:anywhere}.main p{margin:0;color:#526783;font-size:5.3px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.customer{margin-top:.45mm!important;color:#075fd7!important;font-weight:700}
      .grid{display:grid;grid-template-columns:1fr 1fr;margin:1mm 1.6mm;border:1px solid #c7d5e7}.grid div{min-height:6.5mm;padding:.65mm;border-right:1px solid #c7d5e7;border-bottom:1px solid #c7d5e7;overflow:hidden}.grid div:nth-child(even){border-right:0}.grid div:nth-last-child(-n+2){border-bottom:0}.grid small{display:block;color:#6d7e98;font-size:3.7px;letter-spacing:.1px}.grid b{font-size:5.6px;overflow-wrap:anywhere}.qty,.box-cell b{font-size:10px!important;color:#075fd7}.unit{font-size:4.2px;color:#526783}
      footer{display:grid;grid-template-columns:1fr auto;gap:.5mm 1mm;align-items:center;padding:.65mm 1.6mm;border-top:1.1px solid #075fd7;min-width:0}.code{display:block;font-size:4.6px;overflow-wrap:anywhere}.payload{grid-column:1/-1;margin:0;font-size:2.8px;line-height:1.1;color:#71809a;overflow-wrap:anywhere}.hint{display:inline-block;padding:.5mm .8mm;color:#fff;background:#075fd7;border-radius:.8mm;font-size:3.7px;font-weight:700;white-space:nowrap}
  ...
  function renderSettings() {
    ...
    return <>
      <Card title="ข้อมูลระบบ"><div className="system-card"><div className="system-logo">KiT<small>DELIVERY DUE CONTROL</small></div><dl><div><dt>ชื่อระบบ</dt><dd>KIT Delivery Due Control</dd></div><div><dt>เวอร์ชัน</dt><dd>v2.8.10</dd></div><div><dt>เขตเวลา</dt><dd>Bangkok, Thailand</dd></div><div><dt>ผู้ดูแล</dt><dd>{user.displayName}</dd></div></dl><div className="system-stats"><p><span>▤</span><b>{fmt(payload.dues.length)}</b><small>Due ทั้งหมด</small></p><p><span>▣</span><b>{fmt(partImages.length)}</b><small>รูปชิ้นงาน</small></p></div></div></Card>
